...
static int _is_child_state_valid(enum child_state n) {
...
static int _is_child_event_valid(enum child_event n) {
  if (!(n >= AAA && n <= BBB))
    return -1;
  return 0;
}

const char *smtp_fsm_event_ntop(enum child_event n) {
...
const char *smtp_fsm_event_ntop_safe(enum child_event n) {
...
int myfsm_advance(struct smtp_fsm *fsm, enum child_event ev, char *errbuf,
                  size_t errlen) {
...
bad_event:
  if (errlen > 0 && errbuf != NULL) {
    snprintf(errbuf, errlen, "Invalid event %s in state %s",
